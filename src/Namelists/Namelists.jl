...
function setfield_helper(terminal::TTYTerminal, nml::T) where {T<:Namelist}
    while true
        print(terminal, BLUE_FG(to_qe(nml)))
        ...
        if !isdone
            while true
                print(terminal, GREEN_FG("Type a field name: "))
                ...
                if hasfield(T, field)
                    print(terminal, GREEN_FG("Type its value: "))
                    try
                        nml = set(
                            nml,
                            PropertyLens{field}(),
                        ...
                        println(terminal, RED_FG("A wrong value is given to! Try a new one!"))
                        continue
                    end
                    break
                end
                # If the field has a wrong name, go back to `"Type a field name: "`.
                println(terminal, RED_FG("Unknown field given! Try again!"))
                continue
            end
            continue
        end
