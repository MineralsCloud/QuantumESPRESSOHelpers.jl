...
!!! info
    The style guide is always a work in progress, and not all QuantumESPRESSOHelpers code
    follows the rules. When modifying QuantumESPRESSOHelpers, please fix the style violations
...
`QuantumESPRESSOHelpers` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/QuantumESPRESSOHelpers.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `QuantumESPRESSOHelpers` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `QuantumESPRESSOHelpers` have
