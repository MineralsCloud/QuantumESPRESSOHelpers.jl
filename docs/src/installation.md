...
[`QuantumESPRESSOHelpers`](https://github.com/MineralsCloud/QuantumESPRESSOHelpers.jl).
...
## Install `QuantumESPRESSOHelpers`
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("QuantumESPRESSOHelpers")
   ```

3. Run

   ```julia-repl
   julia> using QuantumESPRESSOHelpers
   ```
...
version of `QuantumESPRESSOHelpers`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/QuantumESPRESSOHelpers.jl"
```

in the second step above.

## Update `QuantumESPRESSOHelpers`
...
our [GitHub repository](https://github.com/MineralsCloud/QuantumESPRESSOHelpers.jl)
for new releases.
Once we release a new version, you can update `QuantumESPRESSOHelpers` by typing

```@repl
using Pkg
Pkg.update("QuantumESPRESSOHelpers")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `QuantumESPRESSOHelpers`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("QuantumESPRESSOHelpers")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `QuantumESPRESSOHelpers`.
